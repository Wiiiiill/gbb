...
public:
	...
	void document(class Window* wnd, class Renderer* rnd, const class Theme* theme) {
		...
		ImGui::BeginChild("@Doc/Ctt", ImVec2(width, 0), false, ImGuiWindowFlags_HorizontalScrollbar | ImGuiWindowFlags_NoNav);
		{
			...
			GBBASIC_ASSERT(_blockStack.empty());
			GBBASIC_ASSERT(_spanStack.empty());
			GBBASIC_ASSERT(_scaleStack.empty());
...
private:
	...
	int enterBlock(MD_BLOCKTYPE type, void* detail, Context* context) {
		...
		case MD_BLOCK_UL:
			...
			break;
		case MD_BLOCK_OL:
			GBBASIC_ASSERT(_listItemCount == 0 && "Wrong data.");
		...
		case MD_BLOCK_LI: {
				if (y == MD_BLOCK_OL) {
					GBBASIC_ASSERT(_listItemCount <= (int)_listItemIndeces.size() && "Wrong data.");
	...
	int enterSpan(MD_SPANTYPE type, void* detail, Context* context) {
		...
		switch (type) {
		case MD_SPAN_EM: {
				ImFont* font = (ImFont*)context->theme->fontBlock_Italic();
				if (_bold > 0)
					font = (ImFont*)context->theme->fontBlock_BoldItalic();
				ImGui::PushFont(font);
		...
		case MD_SPAN_A: {
				...
				context->hrefSize = Math::min((size_t)a->href.size, GBBASIC_COUNTOF(context->href));
				GBBASIC_ASSERT(a->href.size <= GBBASIC_COUNTOF(context->href));
				if (context->hrefSize == GBBASIC_COUNTOF(context->href)) {
		...
		case MD_SPAN_IMG: {
				...
				if (it == _images.end()) {
					...
					tex = Theme::createTexture(context->renderer, str.c_str(), nullptr);
	...
	int leaveSpan(MD_SPANTYPE type, void* /* detail */, Context* context) {
		...
		case MD_SPAN_EM:
			...
			ImGui::SetWindowFontScale(_scaleStack.top());
