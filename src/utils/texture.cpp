...
public:
	...
	virtual const Indexed::Ptr palette(bool createIfNecessary, bool assignNewCreated) override {
		if (_palette)
			return _palette;
		...
		Indexed::Ptr palette(Indexed::create(plt->ncolors));
		for (int i = 0; i < plt->ncolors; ++i) {
			const SDL_Color &pcol = plt->colors[i];
			const Colour col(pcol.r, pcol.g, pcol.b, pcol.a);
			palette->set(i, &col);
		}
		...
		if (assignNewCreated) {
			_palette = palette;
			if (_palette) {
				SDL_Palette* plt = (SDL_Palette*)_palette->pointer();
	...
	virtual int paletteCount(void) const override {
		if (!_paletted)
			return 0;
	...
	virtual Colour paletteAt(int index) const override {
		if (!_paletted)
			return Colour();
		...
		const SDL_Color &pcol = plt->colors[index];

		return Colour(pcol.r, pcol.g, pcol.b, pcol.a);
	}
	virtual bool paletteAt(int index, const Colour &col) override {
		if (!_paletted)
			return false;
		...
		SDL_Color &pcol = plt->colors[index];
		pcol.r = col.r;
		pcol.g = col.g;
		pcol.b = col.b;
		pcol.a = col.a;

		SDL_SetPaletteColors(plt, &pcol, index, 1);
	...
	virtual bool resize(class Renderer* rnd, int expWidth, int expHeight) override {
		...
		const int bytes = SDL_BYTESPERPIXEL(format);
		switch (_usage) {
		case STREAMING: {
				// Read the old pixels.
				void* raw = nullptr;
				...
				Colour* pixels = new Colour[_width * _height];
				memcpy(pixels, raw, _width * _height * sizeof(Colour));
				SDL_UnlockTexture(_texture);
				SDL_DestroyTexture(_texture);

				// Create a new texture.
				_texture = SDL_CreateTexture(renderer, format, SDL_TEXTUREACCESS_STREAMING, expWidth, expHeight);
				GBBASIC_ASSERT(_texture);
		...
		case TARGET: {
				...
				const int size = Math::max(expWidth * expHeight, oldw * oldh);
				Colour* pixels = new Colour[size];
				memset(pixels, 0, size * sizeof(Colour));
				const SDL_Rect rect = { 0, 0, oldw, oldh };
				{
					GBBASIC_RENDER_TARGET(rnd, this)
#if SDL_VERSION_ATLEAST(2, 0, 12)
	...
	virtual int toBytes(class Renderer* rnd, Byte* pixels) override {
		int result = _width * _height * sizeof(Colour);
		...
		case TARGET: {
				...
				memset(pixels, 0, _width * _height * sizeof(Colour));
