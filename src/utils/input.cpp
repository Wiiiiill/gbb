...
private:
	...
	struct Touch {
		...
		Touch(int x_, int y_, bool b0, bool b1, bool b2, int wheelX_, int wheelY_) : x(x_), y(y_), wheelX(wheelX_), wheelY(wheelY_) {
			buttons[0] = b0;
			buttons[1] = b1;
			buttons[2] = b2;
		}
	};
	typedef std::vector<Touch> TouchStates;
...
private:
	...
	TouchStates _touchStatesNative;
...
public:
	...
	virtual bool pressed(Button &btn) override {
		...
		for (int i = 0; i < n; ++i) {
			...
			for (int j = 0; j < m; ++j) {
				...
				const SDL_bool hasInit = SDL_TRUE;
#endif /* SDL_VERSION_ATLEAST(2, 0, 6) */
	...
	virtual void update(
		...
		do {
			...
			for (int m = 0; m < n; ++m) {
				...
				while ((int)_touchStatesNative.size() < f)
					_touchStatesNative.push_back(Touch(-1, -1, false, false, false, 0, 0));
				for (int i = 0; i < f; ++i) {
					SDL_Finger* finger = SDL_GetTouchFinger(tid, i);
					if (!finger)
						continue;
					Touch &touch = _touchStatesNative[i];
					if (touch.buttons[0])
						continue;
					touch = Touch(
			...
			if (!hasTouch) {
				...
				if (_touchStatesNative.empty())
					_touchStatesNative.push_back(Touch(-1, -1, false, false, false, 0, 0));
				Touch &touch = _touchStatesNative[0];
	...
	virtual int updateOnscreenGamepad(
		...
		const float btnSmallRadius = btnRadius * 0.7f;
		const float btnBigRadius = btnRadius * 1.25f;
		...
		const ImVec2 posSelect( // Select.
			wnd->Pos.x + wndSize.x * 0.5f - 25.0f,
			wnd->Pos.y + wndSize.y * 0.85f
		);
		const ImVec2 posStart( // Start.
			wnd->Pos.x + wndSize.x * 0.5f + 25.0f,
			wnd->Pos.y + wndSize.y * 0.85f
		);
		ImVec2 posA( // A.
		...
		if (NOBLOCK) {
			...
			const ImVec4 ranges[BUTTON_COUNT] = {
				...
				ImVec4(posDpad.x + dpadCheckRadius * 1.0f, posDpad.y,                          dpadCheckRadius, 0.0f),
				ImVec4(posSelect.x,                        posSelect.y,                        btnSmallRadius,  0.0f),
				ImVec4(posStart.x,                         posStart.y,                         btnSmallRadius,  0.0f),
				ImVec4(posA.x,                             posA.y,                             btnBigRadius,    0.0f),
				ImVec4(posB.x,                             posB.y,                             btnBigRadius,    0.0f)
			};
...
#if defined GBBASIC_DEBUG
						drawList->AddCircle(ImVec2((float)x / scale_, (float)y / scale_), 3.0f, ImColor(255, 0, 0), 15);
#endif /* GBBASIC_DEBUG */
		...
		// Render.
		constexpr const float shadowOffset = 2.0f;
		drawList->AddCircle(posDpad, dpadRadius, WHITE, 19); // D-pad.
		drawList->AddCircle(posDpad, dpadRadius - shadowOffset, BLACK, 19);
		if (pad[LEFT]) { // Left.
		...
		if (pad[SELECT]) { // Select.
			drawList->AddRectFilled(ImVec2(posSelect.x - 20.0f, posSelect.y - 8.0f), ImVec2(posSelect.x + 20.0f, posSelect.y + 8.0f), GRAY);
		}
		drawList->AddRect(ImVec2(posSelect.x - 20.0f, posSelect.y - 8.0f), ImVec2(posSelect.x + 20.0f, posSelect.y + 8.0f), WHITE);
		drawList->AddRect(ImVec2(posSelect.x - 20.0f + shadowOffset, posSelect.y - 8.0f + shadowOffset), ImVec2(posSelect.x + 20.0f - shadowOffset, posSelect.y + 8.0f - shadowOffset), BLACK);
		if (pad[START]) { // Start.
			drawList->AddRectFilled(ImVec2(posStart.x - 20.0f, posStart.y - 8.0f), ImVec2(posStart.x + 20.0f, posStart.y + 8.0f), GRAY);
		}
		drawList->AddRect(ImVec2(posStart.x - 20.0f, posStart.y - 8.0f), ImVec2(posStart.x + 20.0f, posStart.y + 8.0f), WHITE);
		drawList->AddRect(ImVec2(posStart.x - 20.0f + shadowOffset, posStart.y - 8.0f + shadowOffset), ImVec2(posStart.x + 20.0f - shadowOffset, posStart.y + 8.0f - shadowOffset), BLACK);
		if (pad[A]) { // A.
			drawList->AddCircleFilled(posA, btnRadius, GRAY, 15);
		}
		drawList->AddCircle(posA, btnRadius, WHITE, 19);
		drawList->AddCircle(posA, btnRadius - shadowOffset, BLACK, 19);
		if (font) {
			drawList->AddText(font, abSize, ImVec2(posA.x - abSize / 4.5f, posA.y - abSize / 2.0f + shadowOffset), BLACK, "A");
		...
		drawList->AddCircle(posB, btnRadius - shadowOffset, BLACK, 19);
		if (font) {
			drawList->AddText(font, abSize, ImVec2(posB.x - abSize / 4.5f, posB.y - abSize / 2.0f + shadowOffset), BLACK, "B");
	...
	virtual bool touch(
		...
		_activeRequests = (Activities)(_activeRequests | TOUCH_ACTIVE);
		...
		if (idx < 0 || idx >= (int)_touchStates1.size())
			return false;

		const Touch &t = _touchStates1[idx];
		if (x)
			*x = t.x;
		if (y)
			*y = t.y;
	...
	virtual bool immediateTouch(
		...
		do {
			...
			for (int m = 0; m < n; ++m) {
				...
				if (tid == 0)
					continue;
				const int f = SDL_GetNumTouchFingers(tid);
				touch_ = Touch(-1, -1, false, false, false, 0, 0);
				for (int i = 0; i < f; ++i) {
					SDL_Finger* finger = SDL_GetTouchFinger(tid, i);
					if (!finger)
						continue;
					...
					touch_.valid = validatePoint(touch_.x, touch_.y, areax, areay, areaw, areah, canvasw, canvash, scale);
					if (touch_.valid) {
						if (i == 0)
							hasTouch = true;
					} else {
						touch_.buttons[0] = touch_.buttons[1] = touch_.buttons[2] = false;
					}

					break; // First finger only.
				}
			}
			if (!hasTouch) {
				ImGuiIO &io = ImGui::GetIO();
				...
				touch_ = Touch(-1, -1, false, false, false, 0, 0);
				const Uint32 btns = SDL_GetMouseState(&touch_.x, &touch_.y);
				touch_.buttons[0] = !!(btns & SDL_BUTTON(SDL_BUTTON_LEFT));
				touch_.buttons[1] = !!(btns & SDL_BUTTON(SDL_BUTTON_RIGHT));
				touch_.buttons[2] = !!(btns & SDL_BUTTON(SDL_BUTTON_MIDDLE));
				touch_.wheelX = (int)io.MouseWheelH;
				touch_.wheelY = (int)io.MouseWheel;
				touch_.valid = validatePoint(touch_.x, touch_.y, areax, areay, areaw, areah, canvasw, canvash, scale);
				if (touch_.valid) {
					hasTouch = true;
				} else {
		...
		if (b2)
			*b2 = touch_.buttons[2];
		if (wheelX)
			*wheelX = touch_.wheelX;
		if (wheelY)
			*wheelY = touch_.wheelY;

		return touch_.valid;
	}
	virtual bool immediateScreenTouch(
		Window* wnd, Renderer*,
		int* x, int* y,
		bool* b0, bool* b1, bool* b2,
		int* wheelX, int* wheelY
	...
	virtual void sync(void) override {
		...
		_touchStates1 = _touchStatesNative;
		if (_onscreenGamepadPressed > 0)
			_touchStates1.clear();
