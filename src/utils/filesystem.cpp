...
bool Path::existsDirectory(const char* path) {
	...
	const std::string osstr = Unicode::toOs(path);
	if (osstr.length() > GBBASIC_MAX_PATH)
		return false;
...
bool Path::isDirectoryWritable(const char* path) {
	if (!path || !(*path))
		return false;

	const std::string prefix = "writable_test_";
	for (int i = 0; i < 100; ++i) { // Try 100 times.
		std::string path_ = prefix + Text::toString(i);
		path_ = combine(path, path_.c_str());

		FileInfo::Ptr fileInfo = FileInfo::make(path_.c_str());
		if (fileInfo->exists())
			continue;

		if (!fileInfo->make()) // Cannot make new file.
			return false;

		if (!fileInfo->remove(false)) // Cannot emove existing one.
			return false;
...
public:
	FileInfoImpl(const char* path, bool split) : _splitted(split) {
		_fullPath = path;
		if (split)
...
FileInfo* FileInfo::create(const char* path, bool split) {
	FileInfoImpl* result = new FileInfoImpl(path, split);
