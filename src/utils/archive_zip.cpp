...
public:
	...
	virtual bool toString(std::string &val, const char* nameInArchive) const override {
		if (_forWriting)
			return false;
	...
	virtual bool fromString(const std::string &val, const char* nameInArchive) override {
		if (!_forWriting)
			return false;
		...
		if (!val.empty())
			zipWriteInFileInZip(_zipFile, (void*)val.c_str(), (unsigned)val.size());
