...
EM_JS(
	void, webFetchPerform, (unsigned id, const char* url, const char* options), {
		if (!Module.__GBBASIC_WEB_FETCH__)
			Module.__GBBASIC_WEB_FETCH__ = { };

		const res = UTF8ToString(url);
		const opt = JSON.parse(UTF8ToString(options));
		const obj = {
		...
		Module.__GBBASIC_WEB_FETCH__[id] = obj;
	}
);
EM_JS(
	bool, webFetchUpdate, (unsigned id), {
		const obj = Module.__GBBASIC_WEB_FETCH__[id];
		if (!obj)
			return false;
...
EM_JS(
	void, webFetchRemove, (unsigned id), {
		...
		obj.cache.forEach((buf) => {
			_free(buf);
		});
		obj.cache = null;

		delete Module.__GBBASIC_WEB_FETCH__[id];
	}
);
