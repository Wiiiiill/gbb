...
/**
 * @brief Conditional variable guard.
 */
template<typename T> class ConditionalVariableGuard {
private:
	T* _ptr = nullptr;
	T _prev;
	T _curr;
...
public:
	ConditionalVariableGuard(bool cond, T* ptr, const T &prev, const T &curr) : _prev(prev), _curr(curr) {
		_ptr = ptr;
...
/**
 * @brief Container comparer.
 */
template<template<typename T, typename A = std::allocator<T> > class Coll, typename Val> bool equals(const Coll<Val> &colA, const Coll<Val> &colB) {
	if (colA.size() != colB.size())
		return false;

	typename Coll<Val>::const_iterator itA = colA.begin();
	typename Coll<Val>::const_iterator itB = colB.begin();
	for (; itA != colA.end() && itB != colB.end(); ++itA, ++itB) {
		if (*itA != *itB)
			return false;
	}
