...
Variant Variant::operator [] (int index) const {
	if (index < 0)
		return nullptr;
	if (_type != OBJECT)
		return nullptr;
	if (!Object::is<IList::Ptr>(_var.object))
		return nullptr;
	IList::Ptr lst = Object::as<IList::Ptr>(_var.object);
	if (!lst)
		return nullptr;
	if (index >= lst->count())
		return nullptr;
...
Variant Variant::operator [] (const char* key) const {
	...
	if (_type != OBJECT)
		return nullptr;
	if (!Object::is<IDictionary::Ptr>(_var.object))
		return nullptr;
	IDictionary::Ptr dict = Object::as<IDictionary::Ptr>(_var.object);
	if (!dict)
		return nullptr;
...
Variant::operator Long (void) const {
	...
	case INTEGER: return _var.integer;
	case LONG: return _var.long_;
	case REAL: return (Long)_var.real;
	case STRING: // Fall through.
	case POINTER: // Fall through.
	case OBJECT: // Fall through.
	default:
...
int Variant::compare(const Variant &other) const {
	...
	case INTEGER:
		...
		return 0;
	case LONG:
		if (_var.long_ < other._var.long_)
			return -1;
		if (_var.long_ > other._var.long_)
			return 1;
...
bool Variant::isNumber(void) const {
	return _type == INTEGER || _type == LONG || _type == REAL;
}
...
std::string Variant::toString(void) const {
	...
	case LONG:
		return Text::toString(_var.long_);
	case REAL:
...
Dictionary* Dictionary::create(const Arguments &arguments) {
	DictionaryImpl* result = new DictionaryImpl();
	for (const Argument &argument : arguments)
		result->set(argument.first, argument.second);
