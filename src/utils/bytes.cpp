...
public:
	...
	virtual const Collection* collection(void) const override {
		return &_collection;
	}
	virtual Collection* collection(void) override {
		return &_collection;
	}
	...
	virtual size_t readBytes(Byte* buf, size_t expSize) override {
		...
		const size_t len = Math::min(rest(), expSize);
		if (len > 0) {
...
#undef _STEP
	...
	virtual int writeInt8(Int8 val) override {
		return write(val);
	}
	virtual int writeUInt8(UInt8 val) override {
		return write(val);
	}
	...
	virtual int writeBytes(Byte val, size_t len) override {
		if (len == 0)
			return 0;

		advance(len);
		memset(_collection.data() + _cursor, val, len);
		_cursor += len;
...
private:
	...
	void advance(size_t len) {
		if (_cursor + len > _collection.size()) {
			...
			_collection.resize(_collection.size() + diff, 0);
		}
	}
};
