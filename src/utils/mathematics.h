...
template<typename T, typename R = Real> struct Vec2 {
	...
	template<typename S> Vec2(const Vec2<S> &other) {
		x = (ValueType)other.x;
		y = (ValueType)other.y;
	}
	...
	template<typename S> Vec2 &operator = (const Vec2<S> &other) {
		x = (ValueType)other.x;
		y = (ValueType)other.y;
...
template<> inline int pow(int base, int exponent) {
	if (exponent == 0)
		return 1;
	if (exponent == 1)
		return base;
...
template<typename T> inline T ceilIntegerTimesOf(T val, T base) {
	auto div = std::div(val, base);
	T result = (T)div.quot;
	if (div.rem)
		++result;
...
template<typename T> inline bool isInteger(T v) {
	T tmp = 0;

	return std::modf(v, &tmp) == 0;
}
...
template<typename S, typename ...T> inline S min(S start, S car, T ...cdr) {
	const S result = min(min(start, car), cdr...);

	return result;
}

template<typename T> inline T max(T left, T right) {
	return std::max(left, right);
}
template<typename S, typename ...T> inline S max(S start, S car, T ...cdr) {
	const S result = max(max(start, car), cdr...);
...
// Point-circle.
template<typename T> inline bool intersects(const Vec2<T> &point, const Circle<Vec2<T> > &circ, bool withEdge = true) {
...
// Point-AABB.
template<typename T, int S> inline bool intersects(const Vec2<T> &point, const Rect<T, S> &rect, bool withEdge = true) {
	if (withEdge)
...
// Circle-circle.
template<typename T> inline bool intersects(const Circle<Vec2<T> > &circ0, const Circle<Vec2<T> > &circ1, bool withEdge = true) {
...
// Circle-AABB.
template<typename T, int S> inline bool intersects(const Circle<Vec2<T> > &circ, const Rect<T, S> &rect, bool withEdge = true) {
...
// AABB-AABB.
template<typename T, int S> inline bool intersects(const Rect<T, S> &rect0, const Rect<T, S> &rect1, bool withEdge = true) {
	...
	return intersects(center1, rect0m, withEdge);
}

// Aliases.
template<typename T, int S> inline bool intersects(const Rect<T, S> &rect, const Vec2<T> &point, bool withEdge = true) {
	return intersects<T, S>(point, rect, withEdge);
}
